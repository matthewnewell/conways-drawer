...
function readOrigin(): Origin | null {
  ...
  try {
    ...
    return v ? (JSON.parse(v) as Origin) : null
  } catch {
    return null
  }
}
...
export default function DepotBackBar() {
  const [origin] = useState<Origin | null>(readOrigin)
  ...
  return (
    <div className="cd-backbar">
      <a className="cd-backbar__link" href={origin.depot + origin.from}>
        ← Conway's Depot
      </a>
      <span className="cd-backbar__where">back to {labelFor(origin.from)}</span>
    </div>
  )
}
